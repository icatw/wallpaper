  ...
  <div class="animate__animated animate__fadeIn">
    <el-card style="margin:  10px -10px;width: 100%;">
      ...
      <el-descriptions class="margin-top" title="当前用户信息" :column="3" size="medium" border>
        ...
        <el-descriptions-item>
          <template slot="label"><i class="el-icon-tickets"></i>登陆地点</template>
          <el-tag size="small">{{ userInfo.loginLocation }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item>
          <template slot="label">邮箱</template>
          <el-tag size="info">{{ userInfo.email }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item>
        ...
        <el-descriptions-item>
          <template slot="label">浏览器</template>
...
export default {
  data() {
    return {
      userInfo: JSON.parse(window.sessionStorage.getItem("currUserInfo"))
    }
  },
  computed: {
    ...mapGetters([
